# Chat Name Colors

Differentiate players in your chat with custom colors!

# Recolor a User
Simply hold down "Shift" and right click a username in chat or in your Friends list!

![Set Color through menu](./screenshots/set-color.png)

![Set Color through friends list](./screenshots/set-color-friends-list.png)

# Settings

Unspecified Users - When enabled, all User's will be given unique names. The 500 most recently seen users are remembered between sessions. When disabled, only Users that you have explicitly set Colors for will be recolored.

Color Your Name - Enables the ability to recolor your name in Chat

Your Name Color - Specify the color your would like to recolor your name to in Chat (If "Color Your Name" is enabled)

Color Entire Message - Color the entire chat message instead of only the username

# ⚠️ Note
Resetting the plugin configuration will clear all saved name colors including any manually set colors.